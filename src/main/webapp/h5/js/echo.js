  ...
  var _pollImages = function () {
    ...
    if (length > 0) {
      for (var i = 0; i < length; i++) {
        var self = store[i];
        if (self && _inView(self)) {//tracy(2014-4-9): add some customize func
		  var defaultImg = self.src;
		  self.onerror = function(){
			this.onerror = null;
			this.src = defaultImg;
		  };
          var _src = self.getAttribute('data-echo');
          if(_src){
            self.src = _src;
            if(self.naturalHeight && self.naturalHeight===0){
              self.src=defaultImg;
            }
          }
